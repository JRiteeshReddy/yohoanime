...
const isBlockedAnime = (anime) => {
  if (!anime) return true;
  const name = (anime.name || '').toLowerCase();
  const id = (anime.id || '').toLowerCase();
  const title = (anime.title || '').toLowerCase();
  return name.includes('panties') || id.includes('panties') || title.includes('panties') ||
         name.includes('disgusted face') || id.includes('disgusted-face');
};
...
export function parseHome(html) {
  ...
  return { 
    ...
    latestEpisodeAnimes: latestEpisodeAnimes.filter(a => !isBlockedAnime(a)), 
    newReleases: newReleases.filter(a => !isBlockedAnime(a)), 
    topUpcomingAnimes: topUpcomingAnimes.filter(a => !isBlockedAnime(a)), 
    top10Animes: { 
      today: today.filter(a => !isBlockedAnime(a)), 
      day: [], 
      week: week.filter(a => !isBlockedAnime(a)), 
      month: month.filter(a => !isBlockedAnime(a)) 
    } 
  };
}
...
export function parseSearchFromHtml(html) {
  ...
  const filtered = animes.filter(a => !isBlockedAnime(a));
  const cur = getCurrentPage($);
  const last = getLastPage($);
  return { animes: filtered, currentPage: cur, totalPages: last, hasNextPage: cur < last, totalCount: null };
}
...
export function parseAzListFromHtml(html) {
  ...
  const cur = getCurrentPage($);
  const last = getLastPage($);
  return { sortOption: 'all', animes: filtered, currentPage: cur, totalPages: last, hasNextPage: cur < last };
}
...
export function parseListPage(html) {
  ...
  const cur = getCurrentPage($);
  const last = getLastPage($);
  return { title, animes: filtered, currentPage: cur, totalPages: last, hasNextPage: cur < last };
}
...
export function parseAnime(html) {
  ...
  return {
    ...
    related: [],
    recommended: recommended.filter(a => !isBlockedAnime(a)),
    seasons: [],
  };
}
...
function cleanEpisodeTitle(title, animeTitle, epNumber) {
  if (!title) return '';
  ...
  const trimSymbols = (s) => s.replace(/^[\s-:=~|\\\/+.*_]+/g, '').replace(/[\s-:=~|\\\/+.*_]+$/g, '').trim();

  if (animeTitle) {
    const escapedAnimeTitle = animeTitle.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
    const animeTitleRegex = new RegExp(`^${escapedAnimeTitle}\\s*`, 'i');
    clean = clean.replace(animeTitleRegex, '').trim();
    clean = trimSymbols(clean);
    
    const cleanAnimeTitle = animeTitle.replace(/[^a-zA-Z0-9\s]/g, '');
    if (cleanAnimeTitle) {
      const escapedClean = cleanAnimeTitle.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
      const cleanAnimeRegex = new RegExp(`^${escapedClean}\\s*`, 'i');
      clean = clean.replace(cleanAnimeRegex, '').trim();
      clean = trimSymbols(clean);
    }
  }
  ...
  const epPatterns = [
    new RegExp(`^episode\\s*0*${epNumber}\\b`, 'i'),
    new RegExp(`^episode\\s*\\d+\\b`, 'i'),
    new RegExp(`^ep\\b[.]?\\s*0*${epNumber}\\b`, 'i'),
    new RegExp(`^ep\\b[.]?\\s*\\d+\\b`, 'i'),
    new RegExp(`^ep-?\\s*0*${epNumber}\\b`, 'i'),
    new RegExp(`^ep-?\\s*\\d+\\b`, 'i'),
    new RegExp(`^e\\s*0*${epNumber}\\b`, 'i'),
    new RegExp(`^0*${epNumber}\\b`),
  ];
  ...
  if (!clean || lower === 'episode' || lower === 'ep' || lower === String(epNumber) || lower === `episode ${epNumber}`) {
    return '';
  }
...
export function parseEpisodesFromJson(data) {
  ...
  return {
    ...
    episodes: episodesList.map(ep => ({
      number: ep.number,
      title: cleanEpisodeTitle(ep.title || '', anime.title, ep.number),
      isFiller: false,
